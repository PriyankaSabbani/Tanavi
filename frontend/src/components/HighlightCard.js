...
// memo — prevents re-renders from the 60 s silent background refresh in TanaviHighlights
const HighlightCard = memo(({ property }) => {
  ...
  const handleViewDetails = () => {
    ...
    // section: 'tanavi' matches the ref-map key prefix used in Home ("tanavi-<id>").
    try {
  ...
  const handleShare = async (e) => {
    ...
    const pid  = property.propertyCode ? `[${property.propertyCode}]` : '';
    const text = `${pid} ${property.title} - ₹${property.price} at ${property.location}. Your trusted partner in finding the perfect property!`;
    if (navigator.share) {
      try { await navigator.share({ title: `${pid} ${property.title}`, text, url: shareUrl }); }
      catch (err) { if (err.name !== 'AbortError') setShowShareMenu(true); }
    } else {
  ...
  return (
    <>
      ...
      <div className="group relative bg-secondary/10 border-2 border-secondary rounded-xl overflow-hidden shadow-md hover:shadow-2xl hover:border-primary transition-all duration-500 h-full">
        ...
        <div className="p-6">
          <h3 className="text-xl font-bold mb-2 text-textDark group-hover:text-primary transition-colors">
            {property.title}
          </h3>
            ...
            <div className="relative">
              ...
              {showShareMenu && (
                <div className="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-xl z-10 py-2">
                  <button onClick={shareToWhatsApp} className="w-full text-left px-4 py-2 hover:bg-gray-100 text-sm">📱 WhatsApp</button>
                  <button onClick={shareToFacebook} className="w-full text-left px-4 py-2 hover:bg-gray-100 text-sm">📘 Facebook</button>
                  <button onClick={copyLink}         className="w-full text-left px-4 py-2 hover:bg-gray-100 text-sm">🔗 Copy Link</button>
                  <button
